<template>
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6 max-w-max mx-auto">
    ...
    <div class="overflow-x-auto">
      <table class="text-left text-xs whitespace-nowrap">
        <thead>
          <tr class="border-b border-gray-200 text-gray-500 bg-gray-50 uppercase text-sm tracking-wide">
            ...
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              ...
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">Kalorien (kcal)</div>
            </th>
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              <span class="text-base">💧</span>
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">Fett</div>
            </th>
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              <span class="text-base">🧈</span>
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">gesättigte Fette</div>
            </th>
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              <span class="text-base">🥖</span>
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">Kohlenhydrate</div>
            </th>
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              <span class="text-base">🍬</span>
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">Zucker</div>
            </th>
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              <span class="text-base">🥬</span>
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">Ballaststoffe</div>
            </th>
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              <span class="text-base">🥩</span>
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">Eiweiß</div>
            </th>
            <th class="px-2 py-1.5 text-center w-[70px] relative group cursor-pointer">
              <span class="text-base">🧂</span>
              <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 hidden group-hover:block bg-gray-800 text-white text-[11px] py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none normal-case tracking-normal shadow-lg">Salz</div>
            </th>
        ...
        <tbody class="divide-y divide-gray-100 text-gray-700">
          <tr v-if="loading" class="animate-pulse">
            <td colspan="11" class="px-2 py-1.5 text-center text-gray-400">Lade Einträge...</td>
          </tr>
          <tr v-else-if="logs.length === 0">
            <td colspan="11" class="px-2 py-6 text-center text-gray-400">
          ...
          <tr v-else v-for="log in logs" :key="log.id" class="hover:bg-gray-50 transition-colors">
            <td class="pl-4 pr-2 py-1.5 w-[240px] h-[60px]">
              <div class="flex items-center space-x-3">
                ...
                <div class="flex flex-col justify-center h-10 overflow-hidden">
                  <span @click="openFoodDetail(log.food)" class="truncate text-gray-900 font-medium leading-tight text-xs cursor-pointer hover:text-blue-600 transition-colors">{{ log.food.name }}</span>
                  <span v-if="log.food.variant" class="truncate text-gray-500 text-[10px] leading-tight mt-0.5">{{ log.food.variant }}</span>
                  <span v-if="log.food.brand" class="truncate text-blue-600/80 text-[9px] uppercase font-bold leading-tight mt-0.5 tracking-wide">{{ log.food.brand.name }}</span>
                </div>
              </div>
            </td>
            <td class="px-2 py-1.5 text-right w-[70px]">
              <span @click="editAmount(log)" class="inline-flex items-center px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-800 hover:bg-blue-100 hover:border-blue-300 cursor-pointer transition-colors text-[11px]" title="Menge anpassen">
                {{ Number(log.amount) }}{{ log.food.measurement_unit }}
              </span>
            </td>
            <td class="px-2 py-1.5 text-center text-gray-900 font-medium w-[70px]">{{ formatNutrient(log.food.calories_p100, log.amount, 'kcal') }}</td>
            <td class="px-2 py-1.5 text-center w-[70px]">{{ formatNutrient(log.food.fat_p100, log.amount, 'g') }}</td>
            <td class="px-2 py-1.5 text-center text-gray-500 w-[70px]">{{ formatNutrient(log.food.sat_fat_p100, log.amount, 'g') }}</td>
            <td class="px-2 py-1.5 text-center w-[70px]">{{ formatNutrient(log.food.carbs_p100, log.amount, 'g') }}</td>
            <td class="px-2 py-1.5 text-center text-gray-500 w-[70px]">{{ formatNutrient(log.food.sugar_p100, log.amount, 'g') }}</td>
            ...
            <td class="px-2 py-1.5 text-center text-gray-500 w-[70px]">{{ formatNutrient(log.food.salt_p100, log.amount, 'g') }}</td>
        ...
        <tfoot v-if="!loading" class="bg-gray-50 font-bold border-t-2 border-gray-200 text-gray-800 text-[11px]">
          <tr>
            <td colspan="1" class="pl-4 pr-2 py-2 text-left">
              <button @click="$emit('add-log', date)" class="text-xs bg-blue-600 text-white hover:bg-blue-700 font-medium py-1 px-3 rounded shadow-sm transition-colors inline-flex items-center">
            ...
            <td class="px-2 py-1.5 text-center text-gray-500 w-[70px]">{{ formatTotalNutrient('sat_fat_p100', 'g') }}</td>
            <td class="px-2 py-1.5 text-center w-[70px]">{{ formatTotalNutrient('carbs_p100', 'g') }}</td>
            <td class="px-2 py-1.5 text-center text-gray-500 w-[70px]">{{ formatTotalNutrient('sugar_p100', 'g') }}</td>
            <td class="px-2 py-1.5 text-center font-bold text-gray-800 w-[70px]">{{ formatTotalNutrient('fiber_p100', 'g') }}</td>
            <td class="px-2 py-1.5 text-center w-[70px]">{{ formatTotalNutrient('protein_p100', 'g') }}</td>
            <td class="px-2 py-1.5 text-center text-gray-500 w-[70px]">{{ formatTotalNutrient('salt_p100', 'g') }}</td>
